...
$('#edituser').on('show.bs.modal', function (event) {
      ...
      if(button.data('row')!=undefined&&button.data('row')!=null){
        ...
        $('.filter-option-inner-inner:eq(2)').text($('#userlist').DataTable().row('#' + i).nodes(i).data()[i].rolename);
       ...
       $('#u-methods').val(methods);
        if( $('#u-methods').val()==4){
            $("#tel_sign").css("color","red");
            $('.filter-option-inner-inner:eq(3)').text("短信认证");
        }else if($('#u-methods').val()==0){
            $('.filter-option-inner-inner:eq(3)').text("静态密码");
            $("#tel_sign").css("color","white");
       }else if($('#u-methods').val()==1){
            $('.filter-option-inner-inner:eq(3)').text("动态口令");
            $("#tel_sign").css("color","white");
        }else if($('#u-methods').val()==2){
            $('.filter-option-inner-inner:eq(3)').text("密码+指纹");
            $("#tel_sign").css("color","white");
        }else if($('#u-methods').val()==3){
            $('.filter-option-inner-inner:eq(3)').text("指纹认证");
            $("#tel_sign").css("color","white");
        }
         /* $('.filter-option-inner-inner').text($('#role_id').val());
...
function clearT(){
    ...
    $('.filter-option-inner-inner:eq(2)').text("运维用户");
    $('.filter-option-inner-inner:eq(3)').text("静态密码");
/*  console.log($('.filter-option-inner-inner:eq(2)').text());
    var rol_id=$('.filter-option-inner-inner:eq(3)').parents("button").attr("data-id");
    console.log(rol_id)*/
    $('#username').val('');
    ...
    $('#role_id').attr("disabled",false);
 /*   var rol_id=$('.filter-option-inner-inner').parents("button").attr("data-id");
    console.log(rol_id);
   if(rol_id==="role_id"){
       $('.filter-option-inner-inner').text("运维用户");
   }*/
    $('#div6').css('display','none');
    $('#u-methods').val(0);
