...
function init() {
	...
	let minDate = year + "-" + month + "-" + day +"/t"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds();
	//日期控件初始化
			...
			}},
			{
				"data": "id", "render": function (data, type, row, meta) {
					if(row.result<=0){
						...
						(row.result>2?'':'<a data-toggle="modal" data-row="'+meta.row+'" class="newcss1" data-target="#modal-withdraw"  style="margin-left: 20px;cursor:pointer;">撤回</a>');
					}
...
$('#modal-primary1').on('show.bs.modal',function (event) {
	...
	$('#authorize_id1').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].id);
	$('#effect_time1').val(time_Format($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].start));
	$('#name').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].name);
	$('#fail_time1').val(time_Format($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].end));
	$('#upload1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].upload===0?false:true);
	$('#download1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].download===0?false:true);
	$('#filemanage1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].filemanage===0?false:true);
	$('#up_clipboard1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].up_clipboard===0?false:true);
	$('#down_clipboard1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].down_clipboard===0?false:true);
	$('#watermark1').prop("checked",$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].watermark===0?false:true);
	$('#description1').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].description)
});
//编辑授权工单
...
$('#modal-primary8').on('show.bs.modal', function (event) {
	...
	$("#modal-primary8 .modal-title").text("关联设备["+$('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].name+"]");
	$('#modal8_id').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].id);
	$.ajax({
...
$('#modal-withdraw').on('show.bs.modal', function (event) {
	let button = $(event.relatedTarget);
	let i = button.data('row');
	$('#withdraw_id').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].id);
	$('#withdraw_status').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].result);
});
...
$('#modal_default8').on('show.bs.modal', function (event) {
	let button = $(event.relatedTarget);
	let i = button.data('row');
	$('#submit_work').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].id);
	$('#submit_status').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].result);
});
...
$('#modal-primary2').on('show.bs.modal',function (event) {
	let button = $(event.relatedTarget);
	let i = button.data('row');
	$('#primary2_id').val($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].id)
	$.ajax({
...
$('#work_approve').on('show.bs.modal',function (event) {
	...
	$('#workordername').text($('#apply_table').DataTable().row('#' + i).nodes(i).data()[i].name);
