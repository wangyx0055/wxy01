...
function checkApp(){
    var flag=true;
    var ser_name=/^\S{1,32}$/;
    var ser_ip= /^([1-9]|[1-9]\d|1\d{2}|2[0-4][0-9]|25[0-5])\.(\d|[1-9]\d|1\d{2}|2[0-4][0-9]|25[0-5])\.(\d|[1-9]\d|1\d{2}|2[0-4][0-9]|25[0-5])\.(\d|[1-9]\d|1\d{2}|2[0-4][0-9]|25[0-5])$/;
...
$('#modal-default4').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget) // Button that triggered the modal
    i = button.data('row');
    $('#appprogram_server_id').val($('#appserver').DataTable().row('#' + i).nodes(i).data()[i].id);
});
//全选删除
...
$('#edit_app').blur(function () {
    if ($("#edit_app").val()==null){
        $('#judge_app').text("应用程序不能为空");
    }else {
        $('#judge_app').text("");
    }
})
...
function getAppProgramByServer(){
